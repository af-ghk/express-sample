...
const server = new OAuth2Server({
  model: {...OAuth, getClient: OAuthClient.getClient} // See https://github.com/oauthjs/node-oauth2-server for specification
});
    ...
    .authorize(request, response, {
      authenticateHandler: {
        handle: async () => {
          ...
          const client = await OAuthClient.OAuthClientsModel.findOne({clientId: client_id});
...
export default {server, authorize, token, test};